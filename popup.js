document.addEventListener("DOMContentLoaded", async () => {
  const {githubToken: githubToken, feedbackEndpoint: feedbackEndpoint} = await chrome.storage.sync.get([ "githubToken", "feedbackEndpoint" ]);
...
document.getElementById("export-feedback").addEventListener("click", async () => {
  ...
  if (reports.length === 0) {
    ...
    setTimeout(() => statusEl.textContent = "", 2e3);
    return;
  }
  try {
  ...
  setTimeout(() => statusEl.textContent = "", 4e3);
});
