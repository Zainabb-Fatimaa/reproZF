const GITHUB_RESERVED_ROOTS = new Set([ "settings", "notifications", "marketplace", "organizations", "orgs", "sponsors", "topics", "trending", "collections", "codespaces", "new", "login", "join", "about", "pricing", "features", "security", "explore", "apps", "site", "support", "contact", "dashboard", "issues", "pulls", "notifications", "account", "enterprise", "customer-stories", "events", "resources", "solutions", "readme", "search", "watching", "stars" ]);
...
function findPaperPublishDateGeneric() {
  const candidates = Array.from(document.querySelectorAll('time, [class*="date" i], [class*="publish" i], [data-testid*="date" i]'));
...
function buildWidget(score, breakdown, meta, description, repoInfo) {
  ...
  el.querySelector(".rs-feedback-submit").addEventListener("click", async () => {
    ...
    const flaggedCheck = checkIdx === "score" ? "overall score" : checkIdx === "other" ? "other/unspecified" : breakdown[Number(checkIdx)].text;
    const report = {
      repo: repoInfo ? `${repoInfo.owner}/${repoInfo.repo}` : null,
      url: location.href,
      score: score,
      flaggedCheck: flaggedCheck,
      note: note || null,
      timestamp: (new Date).toISOString()
    };
    try {
      const {rs_feedback: rs_feedback = []} = await chrome.storage.local.get("rs_feedback");
      ...
      const sendNote = !sendResponse ? "" : sendResponse.skipped ? "" : sendResponse.ok ? " Sent to the developer." : " (Send failed, saved locally only.)";
...
async function run() {
  ...
  if (location.hostname === "github.com") {
    ...
    if (arxivId) {
      ...
      const ssResponse = await sendMessage({
        type: "LOOKUP_SEMANTIC_SCHOLAR",
        arxivId: arxivId
      });
      loadingEl.remove();
  ...
  } else if (location.hostname === "www.science.org" || location.hostname === "www.researchgate.net") {
    ...
    if (!repoInfo) {
      ...
      const pwcResponse = await sendMessage({
        type: "LOOKUP_PAPERS_WITH_CODE_BY_TITLE",
        title: document.title
      });
      if (pwcResponse && pwcResponse.ok && pwcResponse.repo) {
        repoInfo = pwcResponse.repo;
        semanticMeta = {
          repoSource: pwcResponse.repoSource
        };
        cameFromPaperPage = true;
      }
    }
    ...
    if (!repoInfo) {
      const el = document.createElement("div");
      el.id = "repro-score-widget";
      el.innerHTML = `<div class="rs-header"><span class="rs-label">No GitHub link found (checked the page and Papers with Code by title). Support for ${location.hostname} is experimental, if this paper does link a repo, please report this as a bug.</span></div>`;
      injectWidget(el);
      return;
    }
  }
  if (!repoInfo) {
    if (semanticMeta) {
      const el = document.createElement("div");
      el.id = "repro-score-widget";
      el.innerHTML = `<div class="rs-header"><span class="rs-label">No linked code repo found (${semanticMeta.citationCount ?? "?"} citations on Semantic Scholar).</span></div>`;
      injectWidget(el);
    } else if (location.hostname === "arxiv.org") {
      const el = document.createElement("div");
      el.id = "repro-score-widget";
      el.innerHTML = `<div class="rs-header"><span class="rs-label">Checked the page, Semantic Scholar, and Papers with Code, no linked GitHub repo found for this paper.</span></div>`;
      injectWidget(el);
    } else {
      removeExistingWidget();
    }
  ...
  const response = await sendMessage({
    ...
    skipResearchGate: cameFromPaperPage
  });
  if (!response || !response.ok) {
    const errorEl = document.createElement("div");
    errorEl.id = "repro-score-widget";
    errorEl.innerHTML = `<div class="rs-header" style="border-color:#c62828"><span class="rs-label">${escapeHtml(response && response.error || "Failed to compute score.")}</span></div>`;
  ...
  if (response.result.notResearchRepo) {
    const el = document.createElement("div");
    el.id = "repro-score-widget";
    el.innerHTML = `<div class="rs-header"><span class="rs-label">${response.result.breakdown[0].text}</span></div>`;
    injectWidget(el);
    return;
  }
